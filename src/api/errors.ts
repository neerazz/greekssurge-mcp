...
export function mapHttpStatus(
...
): GreeksSurgeApiError | undefined {
  if (status === 401)
    return new GreeksSurgeApiError(
      "AUTH_REQUIRED",
      "GreeksSurge authentication is required.",
    );
  if (status === 403)
    return new GreeksSurgeApiError(
      "TIER_REQUIRED",
      "This GreeksSurge account tier cannot access the requested data.",
    );
  if (status === 429) {
  ...
  if (status >= 500)
    return new GreeksSurgeApiError(
      "UPSTREAM_UNAVAILABLE",
      "GreeksSurge is temporarily unavailable.",
    );
  return undefined;
}
