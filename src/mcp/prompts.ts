...
function filterLines(
  ...
  if (supplied.length === 0)
    return "No filters were supplied. Pass no filter arguments; retain the requested page and limit arguments, and say the screen is unfiltered.";
  return [
...
export function registerGreeksSurgePrompts(
    ...
    () =>
      userMessage(
        [
          ...
          "",
          "Report tier, isLifetimeFree, features, and premiumMasked verbatim. Do not infer",
          "a tier-to-tool entitlement map from those fields. A tool is verified accessible only",
          "after its own live tool call succeeds; otherwise report the returned auth/tier error.",
          "Call out explicitly if premiumMasked is true, because masked values can read as zero or blank.",
          "",
          GUARDRAIL,
        ].join("\n"),
      ),
  ...
  options.register(
    "assignment_review",
    {
      ...
      argsSchema: {
        outcome: optionalArg(
          "Outcome filter: ALL, OTM, BTC, or ASSIGNED. Omit for all outcomes.",
        ),
    ...
    (args) =>
      userMessage(
        [
          ...
          "",
          "1. Call get_available_filters and validate any supplied outcome against its live",
          "   outcomes list. Accepted production values are ALL, OTM, BTC, and ASSIGNED.",
          "   Stop on an invalid value rather than translating it.",
          "2. Call list_trade_history with limit 50.",
          "",
          filterLines([
    ...
    () =>
      userMessage(
        [
          ...
          "",
          "Then:",
          "- If the watchlist is empty, say so directly, skip the cross-reference, and present",
          "  up to ten ideas ordered by probOtm, then buffer, then capital fit, with ROI as the",
          "  reward field rather than the decision rule. Direct the user to screen_ideas or",
          "  cash_secured_put_plan for a full comparison.",
    ...
    (args) =>
      userMessage(
        [
          ...
          "4. Call list_trade_ideas with page 1, limit 100, and exactly the validated filters below.",
          "   Read pagination.pages and fetch every page up to five pages total using the same",
          "   filters and limit 100. If more than five pages exist, state that the recommendation",
          "   is bounded to the first 500 returned ideas rather than implying a complete screen.",
          "5. Preselect up to three specific idea contracts—not just tickers—using probOtm,",
          "   buffer, capital fit, and ROI as reward. Preserve each contract's id, displaySymbol,",
          "   expiry, and strike. Call analyze_ticker once per unique ticker, then join the exact",
          "   selected contract to analyze_ticker.openIdeas by `id`; exclude a contract if the id",
          "   is missing from the analysis instead of borrowing another strike or expiry.",
          "",
          filterLines([
            ["ticker", args.ticker],
            ["expiry", args.expiry],
          ...
          "the ITM/assignment-risk proxy, premium per risk unit, sampled historical assignment",
          "rate from explicit ASSIGNED rows, net premium,",
          ...
          "A. Screen conditions — market status, filters, access limits, candidate count, the",
          "   list_trade_ideas envelope retrievedAt timestamp, and its data.cached flag. If",
          "   retrievedAt is missing or the feed cannot be bounded, recommend waiting.",
          "B. Comparison table — one row per exact id/displaySymbol contract, including expiry",
          "   and strike, with the decisive returned measurements. Treat analyze_ticker `spot`",
          "   as the idea-time triggerPrice, not a current quote.",
    ...
    (args) =>
      userMessage(
        [
          ...
          "cash-secured-put first wheel stage. Cover break-even, the ITM/assignment-risk proxy",
          "(100 - probOtm, not assignment probability), observed explicit assignments and",
