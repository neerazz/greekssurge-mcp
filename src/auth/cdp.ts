...
export class CdpSession {
  ...
  async sendCommand(method: string, params: Record<string, unknown> = {}, timeoutMs = 5_000): Promise<unknown> {
  ...
  private readonly onError = (error: unknown) => {
    for (const pending of this.pending.values()) pending.reject(error instanceof Error ? error : new Error('CDP transport failed'));
    this.pending.clear();
  };
}

export function connectCdp(webSocketDebuggerUrl: string): CdpSession {
  return new CdpSession(new WebSocket(webSocketDebuggerUrl) as unknown as CdpTransport);
}

export async function listTabs(port: number, fetchImpl: typeof fetch = fetch): Promise<CdpTab[]> {
...
export function findGreeksSurgeTab(tabs: CdpTab[]): CdpTab | undefined {
  return tabs.find((tab) => tab.type === 'page' && tab.url.startsWith('https://csp.greekssurge.com/') && tab.webSocketDebuggerUrl);
}

export async function readGsTokenFromTab(session: CdpSession, tabUrl: string): Promise<string | undefined> {
  const origin = new URL(tabUrl).origin;
  if (origin !== 'https://csp.greekssurge.com') {
    throw new Error('Refusing to evaluate CDP JavaScript outside the exact GreeksSurge origin.');
  }
  ...
  return typeof result.result?.value === 'string' && result.result.value.length > 0 ? result.result.value : undefined;
}
