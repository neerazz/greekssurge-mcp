...
2. Local auth flow: locates an authenticated exact-origin GreeksSurge tab in BrowserOS and reads its existing session through BrowserOS's loopback-only DevTools endpoint.
3. Token store: validates the imported token through `/api/auth/me` and stores it atomically under the user's OS config directory.
...
No Google password collection occurs. BrowserOS remains the owner of the browser login. The CLI accepts a session only from an exact `https://csp.greekssurge.com` page whose DevTools WebSocket is loopback-only, validates it through `/api/auth/me`, and only then writes it. Tokens never pass through CLI arguments, stdout, logs, clipboard, or manual copy/paste. The default token path is OS-specific and user-scoped:
