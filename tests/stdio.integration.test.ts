...
async function withFixtureApi() {
  const server = createServer((req: IncomingMessage, res: ServerResponse) => {
    if (req.url?.startsWith('/api/status')) {
      ...
      res.end(JSON.stringify({ market: 'open', asOf: '2026-07-26T15:30:00.000Z', timezone: 'America/New_York' }));
      return;
    }
...
function send(proc: ChildProcessWithoutNullStreams, id: number, method: string, params: Record<string, unknown> = {}) {
...
describe('stdio CLI integration', () => {
  it('initializes, lists tools, and invokes one mocked public tool with JSON-RPC-only stdout', async () => {
    ...
    const proc = spawn(process.execPath, ['dist/cli.js', 'serve'], {
      cwd: process.cwd(),
      env: { ...process.env, GREEKSSURGE_API_BASE_URL: api.url, GREEKSSURGE_AUTH_ISSUER: 'https://csp.greekssurge.com' },
    ...
    try {
      send(proc, 1, 'initialize', { protocolVersion: '2025-11-25', capabilities: {}, clientInfo: { name: 'integration', version: '1.0.0' } });
      ...
      expect(call).toMatchObject({ id: 3, result: { structuredContent: { data: { market: 'open' } } } });
    } finally {
      proc.kill('SIGTERM');
