...
describe('GreeksSurge upstream schemas', () => {
  ...
  it.each(cases)('accepts the sanitized %s fixture', async (schemaName, fileName) => {
  ...
  it('raises endpoint-specific drift errors when required keys change', async () => {
    ...
    expect(() => parseUpstream('status', raw)).toThrow(/Upstream contract changed for status/);
  });

  it('defines safe DTOs separately from raw upstream shapes', async () => {
    const account = toAccountDto(parseUpstream('authMe', await fixture('auth-me')));
    const status = toStatusDto(parseUpstream('status', await fixture('status')));
    const ideas = toIdeaDtos(parseUpstream('ideas', await fixture('ideas')));

    expect(account).toEqual({ tier: 'premium', subscriptionStatus: 'active', features: ['ideas', 'history'], premiumMasked: false });
    expect(status).toMatchObject({ market: 'open', source: 'https://csp.greekssurge.com' });
    expect(ideas[0]).toMatchObject({ id: 'idea_1', ticker: 'AAPL', isMasked: false });
    expect(JSON.stringify({ account, status, ideas })).not.toContain('user_fixture_123');
  });
});
