...
describe('FileTokenStore', () => {
  it('prefers an environment token without writing it to disk', async () => {
    ...
    const store = new FileTokenStore({ tokenPath, env: { GREEKSSURGE_TOKEN: 'env-token' } });
  ...
  it('returns undefined when no token exists', async () => {
    const store = new FileTokenStore({ tokenPath: await tempTokenPath(), env: {} });
  ...
  it('writes atomically with POSIX 0600 permissions where supported', async () => {
    ...
    expect(JSON.parse(await readFile(tokenPath, 'utf8'))).toEqual({ token: 'site-token' });
