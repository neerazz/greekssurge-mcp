...
describe('loadConfig', () => {
  it('loads safe defaults for local stdio mode', () => {
    ...
    expect(config.authIssuerUrl.toString()).toBe('https://csp.greekssurge.com/');
  ...
  it('rejects invalid URLs and ports', () => {
    expect(() => loadConfig({ GREEKSSURGE_API_BASE_URL: 'ftp://bad.test' })).toThrow(
      /GREEKSSURGE_API_BASE_URL/,
    );
